...
index 3d7810ee8eabd501a3a72b62b890b91975a01ee4..d5603e46a755d48b0e63a543b1c0972f5991f45a 100644
--- a/app.py
+++ b/app.py
@@ -1,105 +1,154 @@
...
+
+async def process_all_tracks_fast(menu_data: List[Dict[str, str]], output_dir: str, voice_code: str, rate_value: str, progress_bar):
     tasks = []
     track_info_list = []
...
+    """Streamlit環境でも安全にコルーチンを実行するヘルパー。"""
+    try:
...
+
+
+def validate_menu_response(text_resp: str) -> List[Dict[str, str]]:
+    start = text_resp.find('[')
+    end = text_resp.rfind(']') + 1
+    if start == -1 or end == 0:
+        raise ValueError("解析結果がJSON配列ではありません。")
+
+    try:
+        menu_data = json.loads(text_resp[start:end])
+    except json.JSONDecodeError as e:
+        raise ValueError(f"JSONの読み取りに失敗しました: {e}")
+
+    if not isinstance(menu_data, list) or not menu_data:
+        raise ValueError("解析結果が空、または配列ではありません。")
+
...
+            continue
+        cleaned.append({"title": str(title), "text": str(text)})
+
+    if not cleaned:
+        raise ValueError("メニュー項目の抽出に失敗しました。")
+    return cleaned
+
...
@@ -152,62 +201,63 @@ def render_preview_player(tracks):
...
@@ -225,145 +275,174 @@ if input_method == "📂 アルバムから":
...
+        size = len(img.getbuffer()) if hasattr(img, "getbuffer") else 0
+        if size > MAX_IMAGE_BYTES:
+            st.warning(f"{img.name} は5MBを超えるためスキップしました。")
+            continue
+        filtered_images.append(img)
+    if len(filtered_images) > MAX_IMAGES:
+        st.warning(f"画像は最大{MAX_IMAGES}枚まで処理します。先頭{MAX_IMAGES}枚のみを使用します。")
+        filtered_images = filtered_images[:MAX_IMAGES]
+    final_image_list = filtered_images
+
...
+    if target_url and final_image_list:
+        st.warning("画像またはURLのどちらか一方のみを選んでください。")
+        st.stop()
...
+                    parts.append({"mime_type": mime_type, "data": f.getvalue()})
             elif target_url:
...
+                    st.warning(f"モデル呼び出しに失敗しました: {e}")
+                    time.sleep(2)
 
-            if not resp: st.error("失敗しました"); st.stop()
+            if not resp:
+                st.error("メニュー解析に失敗しました。しばらく待ってから再度お試しください。")
+                st.stop()
...
-            menu_data = json.loads(text_resp[start:end])
+            menu_data = validate_menu_response(resp.text)
...
+            generated_tracks = run_async_task(
